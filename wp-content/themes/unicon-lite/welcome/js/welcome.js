...
jQuery(document).ready(function ($) {
	...
	$(".install").on('click', function (e) {
    	...
    	var ajaxurl = uniconliteWelcomeObject.ajaxurl;

		$.ajax({
			type: 'POST',
			url: ajaxurl,
			data: {
				action: 'unicon_lite_plugin_installer',
				plugin: plugin,
				plugin_file: plugin_file,
				nonce: uniconliteWelcomeObject.admin_nonce,
			},
			success: function(response) {
		   		...
		   		if(response == 'success'){
				   		...
				   		el.html(uniconliteWelcomeObject.installed_btn);
	...
	$('.install-offline').on('click', function (e) {
		...
		$.ajax({
			...
			url: ajaxurl,
			data: {
				action: 'unicon_lite_plugin_offline_installer',
	...
	$(".activate").on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'unicon_lite_plugin_activation',
	   			plugin: plugin,
	   			nonce: uniconliteWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
	   		success: function(response) {
		   		if(response){
	...
	$('.activate-offline').on('click', function (e) {
		...
		$.ajax({
	   		...
	   		url: ajaxurl,
	   		data: {
	   			action: 'unicon_lite_plugin_offline_activation',
	   			plugin: plugin,
	   			nonce: uniconliteWelcomeObject.activate_nonce,
	   			dataType: 'json'
	   		},
